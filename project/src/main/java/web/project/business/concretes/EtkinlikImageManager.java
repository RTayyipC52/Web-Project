...
public class EtkinlikImageManager implements EtkinlikImageService{
	...
	@Override
    public DataResult<EtkinlikImage> getById(int id) {
        if (this.etkinlikImageDao.findById(id).isEmpty()) {
            return new ErrorDataResult<EtkinlikImage>("Girdiğiniz sayıda resim yoktur");
        }else {
            return new SuccessDataResult<EtkinlikImage>
                    (this.etkinlikImageDao.getById(id), "Id'sine göre resim listelendi");
        }
    }
	 ...
	 @Override
	    public Result upload(int id, MultipartFile file) {
	        ...
	        image.setEtkinlik(etkinlikService.getById(id).getData()); //Id'yi veritabanında image'ın etkinlikId'sine setler
	    ...
	    public Result delete(int id) {
	    	...
	    	EtkinlikImage image = getById(id).getData(); //Girilen Id'ye ait veriyi getirir
	        ...
	        etkinlikImageDao.deleteById(id); //Id'yi veritabanından siler
